...
import { loadPipelineConfig, loadGlobalConfig, listPipelineNames } from "./config/loader.js";
  ...
  for (const name of names) {
    ...
    try {
      ...
      if (result.status === RUN_STATUS_INTERRUPTED) {
        ...
        anyLockHeld = true;
      } else if (result.status !== RUN_STATUS_OK && result.status !== "halted" && result.status !== "skipped") {
        anyError = true;
      }
    } catch (err) {
      logger.error(`${name}: ${err instanceof Error ? err.message : String(err)}`);
      anyError = true;
    }
  }
...
async function cmdStateShow(args: ParsedArgs): Promise<number> {
  ...
  if (stepArg !== undefined) {
    ...
    const stepName = !isNaN(idx) && idx >= 0 && idx < keys.length ? keys[idx] : stepArg;
...
async function cmdStateHistory(args: ParsedArgs): Promise<number> {
  ...
  const format = (flagStr(args, "format") ?? "table") as "table" | "json" | "tsv";
  ...
  const filtered = stepFilter
    ? entries.map(e => ({
        ...e,
        stepOutputs: e.stepOutputs[stepFilter] !== undefined
...
async function cmdStateDiff(args: ParsedArgs): Promise<number> {
  ...
  if (entries.length < 2) {
    output.printHelp("Not enough history entries for diff (need at least 2 runs).");
    return EXIT_SUCCESS;
  }
...
async function cmdStateClear(args: ParsedArgs): Promise<number> {
  const name = args.positional[0];
  if (!name) {
    output.printHelp("Usage: aspyn state clear <name> [--step <s>] [--wipe-history]");
    return EXIT_USAGE;
  }
  ...
  logger.info(`State cleared for "${name}"${step ? ` (step: ${step})` : ""}${wipeHistory ? " (history wiped)" : ""}`);
  return EXIT_SUCCESS;
}
...
async function cmdValidate(args: ParsedArgs): Promise<number> {
  ...
  if (jsonFormat) {
    ...
    output.markExternalOutput();
    return results.some(r => r.errors.length > 0) ? EXIT_STEP_ERROR : EXIT_SUCCESS;
  }
  ...
  for (const r of results) {
    ...
    const msg = r.errors.length > 0 ? r.errors.map(e => e.message).join("; ") : undefined;
...
async function cmdInit(args: ParsedArgs): Promise<number> {
  ...
  const steps = CANONICAL_STEP_NAMES.map(s => {
    const type = s === "input" ? "http" : s === "parse" ? "jsonpath" : s === "check" ? "expr" : "log";
